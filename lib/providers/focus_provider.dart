...
class FocusProvider extends ChangeNotifier {
  ...
  bool _isRunning = false; // 是否计时中
  bool _isRest = false; // false:正在专注，true:正在休息
  bool _earlyExit = false; // 是否提前退出
  DateTime _sessionStartTime = DateTime.now(); // 整个专注 - 休息会话的开始时间
  int _focusDuration = 0; // 专注时长
  final int _restDuration = 5; // 休息时长默认为 5 分钟
  ...
  Future<void> _saveInterruptedSession() async {
    try {
      ...
      if (_isRest) {
        ...
        // 验证 elapsedSeconds 是否合理（不超过设定的专注时间对应的秒数）
